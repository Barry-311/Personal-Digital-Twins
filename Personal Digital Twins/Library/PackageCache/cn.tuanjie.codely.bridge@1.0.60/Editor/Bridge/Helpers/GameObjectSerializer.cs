...
namespace UnityTcp.Editor.Helpers
{
    ...
    public static class GameObjectSerializer
    {
        ...
        {
            // --- Add Early Logging --- 
            // CodelyLogger.Log($"[GetComponentData] Starting for component: {c?.GetType()?.FullName ?? "null"} (ID: {c?.GetInstanceID() ?? 0})");
            // --- End Early Logging ---
            ...
            if (componentType == typeof(Transform))
            {
                ...
                // CodelyLogger.Log($"[GetComponentData] Manually serializing Transform (ID: {tr.GetInstanceID()})");
