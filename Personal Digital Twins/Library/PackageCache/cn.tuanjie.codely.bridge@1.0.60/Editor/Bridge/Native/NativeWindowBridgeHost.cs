...
namespace UnityTcp.Editor.Native
{
    ...
    internal static partial class NativeWindowBridgeHost
    {
        ...
        private static void OnBeforeAssemblyReload()
        {
            ...
            try { SendDataChannelMessage("{\"type\":\"domain_reload\"}"); }
            catch (Exception) { }

            // Close floating capture window
            if (s_OffscreenAutoFloatingWindow)
            {
                if (s_OffscreenFloatingWindowInstanceId != 0)
                    SessionState.SetInt(kOrphanedFloatingWindowKey, s_OffscreenFloatingWindowInstanceId);
            ...
            // Release C# composite state (native offscreen stays active)
            if (s_CompositeActive)
            {
                ReleaseCompositeState();
                CodelyLogger.Log("[NWB] Closed composite capture windows before assembly reload");
            }
        ...
        private static void TryRestoreOffscreenAfterReload()
        {
            ...
            int orphanId = SessionState.GetInt(kOrphanedFloatingWindowKey, 0);
            if (orphanId != 0)
            {
                SessionState.SetInt(kOrphanedFloatingWindowKey, 0);
                foreach (var win in Resources.FindObjectsOfTypeAll<EditorWindow>())
                {
                    if (win != null && win.GetInstanceID() == orphanId)
                    {
                        try
                        {
                            win.Close();
            ...
            try
            {
                ...
                {
                    CodelyLogger.Log("[NWB] Domain reload detected active composite offscreen; waiting for browser layout refresh.");
                    return;
                }
...
#endif
        private static int s_OffscreenFloatingWindowInstanceId;
#if UNITY_EDITOR_WIN
        ...
        private static void MakeAllWindowsTransparent()
        {
            ...
            try
            {
                ...
                foreach (var hwnd in hwnds)
                {
                    ...
                    // Save original window position before moving off-screen.
                    // If the window is already at the off-screen sentinel position
                    // (e.g. domain reload scenario), do NOT overwrite the saved
                    // position — it would record the hidden position as "original".
                    GetWindowRect(hwnd, out RECT origRect);
                    bool alreadyAtSentinel = (origRect.Left <= -30000 && origRect.Top <= -30000);
                    ...
                    {
                        LogVerbose($"[NWB-Win] Hide: hwnd=0x{hwnd.ToInt64():X} already off-screen at ({origRect.Left},{origRect.Top}), not saving as original pos");
                    }
                    ...
                    // Instead, use WS_EX_TRANSPARENT for click-through and
                    // move the window off-screen to hide it visually.
                    SetWindowLongW(hwnd, GWL_EXSTYLE,
                        exStyle | WS_EX_TRANSPARENT);

                    // Move to far off-screen position (-32000, -32000) while
                    // keeping the window "visible" for GrabPixels to work.
                ...
                LogVerbose($"[NWB-Win] Made {hwnds.Count} HWNDs hidden (off-screen + click-through, no WS_EX_LAYERED)");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (var hwnd in hwnds)
                {
                    ...
                    if (!s_OriginalWindowRects.ContainsKey(hwnd))
                    {
                        ...
                        // Only save if the window is NOT already at the off-screen
                        // sentinel position. After domain reload, windows are
                        // already hidden — saving that position as "original" would
                        // prevent RestoreAllWindowsVisible from bringing them back.
                    ...
                    // Keep the live HWND size while moving it off-screen.
                    // Skip SetWindowPos entirely when already at the correct
                    // position/size to avoid generating WM_WINDOWPOSCHANGED
                    // which triggers GUIView::ApplicationSetSceneRepaintDirty
                    // and can interfere with GameView's internal layout.
                    ...
                    if (s_OriginalWindowRects.TryGetValue(hwnd, out RECT originalRect))
                    {
                        int ow = originalRect.Right - originalRect.Left;
                        int oh = originalRect.Bottom - originalRect.Top;
                        if ((ow != w2 || oh != h2) && (s_HideCallCount <= 8 || s_HideCallCount % 200 == 0))
                        {
                            LogVerbose($"[NWB-Win] Reapply keep live size: hwnd=0x{hwnd.ToInt64():X} live={w2}x{h2} original={ow}x{oh} alreadyOff={alreadyOffscreen}");
                        }
                    }
                    ...
                    if (!alreadyOffscreen)
                    {
                        SetWindowPos(hwnd, IntPtr.Zero, -32000, -32000, w2, h2,
                            SWP_NOACTIVATE | SWP_NOZORDER);
                        if (s_HideCallCount <= 8 || s_HideCallCount % 200 == 0)
                            LogVerbose($"[NWB-Win] Reapply moved off-screen: hwnd=0x{hwnd.ToInt64():X} from=({liveRect.Left},{liveRect.Top}) size={w2}x{h2}");
                    }
                    s_TransparentHwnds.Add(hwnd);
                }
            }
        ...
        private static void RestoreAllWindowsVisible(IntPtr excludeNSWindow = default)
        {
            ...
            try
            {
                ...
                // Move windows back to their original positions and bring to
                // top. SWP_FRAMECHANGED ensures Windows/DWM refreshes the
                // hit-test region after removing WS_EX_TRANSPARENT.
                // SAFETY: if the saved rect is still at the off-screen sentinel
                // position (domain reload lost the real position), use
                // SWP_NOMOVE to leave the window at whatever position Unity's
                // managed layer set, but still apply SWP_SHOWWINDOW.
                foreach (var hwnd in hwnds)
                {
                    if (s_OriginalWindowRects.TryGetValue(hwnd, out RECT r))
                    {
                        bool savedIsOffscreen = (r.Left <= -30000 && r.Top <= -30000);
                        if (savedIsOffscreen)
                        {
                            CodelyLogger.LogWarning($"[NWB-Win] Restore hwnd=0x{hwnd:X}: saved rect ({r.Left},{r.Top}) is off-screen, using ShowWindow fallback");
                        ...
                        else
                        {
                            ...
                            // Use SW_SHOW instead of SWP_SHOWWINDOW alone to
                            // ensure WS_VISIBLE is set even for windows that
                            // lost it during streaming.
                            ShowWindow(hwnd, SW_SHOW);
                    ...
                    {
                        // No saved rect: window was either created during
                        // streaming or domain reload lost the saved data.
                        // Use ShowWindow + SWP_NOMOVE to bring it on-screen
                        // at its current managed position.
                        CodelyLogger.LogWarning($"[NWB-Win] Restore hwnd=0x{hwnd:X}: no saved rect, using ShowWindow fallback");
                        ShowWindow(hwnd, SW_SHOW);
                        SetWindowPos(hwnd, HWND_TOP, 100, 100, 0, 0,
                            SWP_NOSIZE | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
                    }
        ...
        private static void EnsureWindowsStayHiddenDuringOffscreen()
        {
            ...
            try
            {
                // Always enforce off-screen positioning. EditorWindow.position
                // setter (used by ApplyPendingOffscreenResizeTarget and Unity
                // internal layout) can move HWNDs back on-screen. With the
                // alreadyOffscreen optimization in ReapplyWindowTransparency,
                // this only calls SetWindowPos when actually needed, avoiding
                // redundant WM_WINDOWPOSCHANGED messages.
                ReapplyWindowTransparency();
        ...
        /// <summary>
        /// Enumerate all top-level windows belonging to the current Unity editor process.
        /// Excludes the streaming mask window HWND so it is never hidden/pushed.
        /// </summary>
        ...
        private static void ShowStreamingMaskWindow()
        {
            try
            {
                // Record existing visible HWNDs (all of them are already off-screen).
                ...
                // main thread, so EnumWindows sees it immediately.
            ...
            try
            {
                ...
                s_OffscreenFloatingWindowInstanceId = 0;
                {
                    try
                    {
                        ...
                        s_OffscreenFloatingWindowInstanceId = floating.GetInstanceID();
                ...
                try
                {
                    ...
                    // Pass 1: Layout triggers ConfigureTargetTexture; Repaint renders
                    target.SendEvent(new Event { type = EventType.Layout });
                    target.SendEvent(new Event { type = EventType.Repaint });
                    // Pass 2: Render again with correctly sized m_TargetTexture
                    target.SendEvent(new Event { type = EventType.Layout });
                    target.SendEvent(new Event { type = EventType.Repaint });
        ...
        private static bool StartCompositeOffscreenCapture(string requestJson, int fps, int width, int height)
        {
            ...
            try
            {
                ...
                ApplyCompositeLayout(request, logicalW, logicalH);
                ...
                CodelyLogger.Log($"[NWB-Composite] Started slots={s_CompositeSlots.Count} canvas={logicalW}x{logicalH} phys={physW}x{physH} @{fps}fps");
                return true;
            }
            catch (Exception ex)
            {
        ...
        private static void ApplyCompositeLayout(CompositeStreamRequest request, int logicalW, int logicalH)
        {
            ...
            // New slots are docked via direct AddTab in CreateCompositeSlot,
            // so deferred docking (EnsureCompositeWorkspaceDocking) is removed.
        ...
        private static CompositeCaptureSlot CreateCompositeSlot(string slotId, string windowTypeName, int logicalW, int logicalH, CompositeRect rect)
        {
            ...
            // For subsequent slots, directly AddTab to the existing DockArea
            // without calling Show(). This avoids creating a temporary
            // ContainerWindow that needs cleanup. Show() + RemoveTab/AddTab
            // left orphan empty DockAreas causing NRE in OldOnGUI and
            // residual black windows after streaming stops.
            ...
            if (existingDockArea != null && !shouldReuseExistingWindow)
            {
                try
                {
                    ...
                    if (addTab != null)
                    {
                        ...
                        addedDirectly = true;
                        LogVerbose($"[NWB-Composite] AddTab {windowTypeName} directly to existing DockArea");
                    }
                }
                catch (Exception ex)
                {
        ...
        private static void ResizeCompositeWorkspaceToCanvas()
        {
            ...
            if (hostWindow == null || s_CompositeLogicalWidth <= 0 || s_CompositeLogicalHeight <= 0)
                return;
        ...
        public static void StopOffscreenCapture()
        {
            ...
            if (hadAutoFloating)
            {
                try
                {
                    ...
                    int floatingInstanceId = s_OffscreenFloatingWindowInstanceId;
                    if (closeTarget == null && floatingInstanceId != 0)
                    {
                        foreach (var win in Resources.FindObjectsOfTypeAll<EditorWindow>())
                        {
                            if (win != null && win.GetInstanceID() == floatingInstanceId)
                            {
                                closeTarget = win;
                                break;
                            }
            ...
            int savedFloatingInstanceId = hadAutoFloating ? s_OffscreenFloatingWindowInstanceId : 0;
            EditorApplication.delayCall += () =>
            {
                if (hadAutoFloating)
                {
                    if (savedFloatingInstanceId != 0)
                    {
                        foreach (var win in Resources.FindObjectsOfTypeAll<EditorWindow>())
                        {
                            if (win != null && win.GetInstanceID() == savedFloatingInstanceId)
                            {
                                try
                                {
                                    win.Close();
            ...
            // clean styles. If any still have WS_EX_TRANSPARENT, force-remove
            // it and bring the window on-screen. This handles edge cases where
            // the primary restore path failed (e.g. domain reload corrupted
            // the saved state).
            try
            {
                ...
                foreach (var hwnd in verifyHwnds)
                {
                    ...
                    if (stillLayered || stillTransparent)
                    {
                        ...
                        // Force-remove WS_EX_TRANSPARENT and WS_EX_LAYERED.
                        int cleanedStyle = style & ~WS_EX_TRANSPARENT & ~WS_EX_LAYERED;
                        SetWindowLongW(hwnd, GWL_EXSTYLE, cleanedStyle);
                        // Bring window on-screen if still at sentinel position.
                        ...
                        if (rect.Left <= -30000 || rect.Top <= -30000)
                        {
                            ...
                            SetWindowPos(hwnd, HWND_TOP, 100, 100, 0, 0,
                                SWP_NOSIZE | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
                            CodelyLogger.LogWarning($"[NWB-Win] POST-STOP AUTO-REPAIR: moved hwnd=0x{hwnd:X} from ({rect.Left},{rect.Top}) to on-screen");
                        }
                        RedrawWindow(hwnd, IntPtr.Zero, IntPtr.Zero,
                            RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_ERASE | RDW_FRAME);
                    }
        ...
        private static void OffscreenUpdateLoop()
        {
            ...
            try
            {
                ...
                {
                    CodelyLogger.Log("[NWB-Offscreen] Native offscreen mode ended, cleaning up C# side");
                    StopOffscreenCapture();
                    return;
                }
                ...
                {
                    double sinceHeartbeat = EditorApplication.timeSinceStartup - s_LastFrontendHeartbeatTime;
                    if (sinceHeartbeat > kFrontendHeartbeatTimeoutSec)
                    {
                        CodelyLogger.LogWarning($"[NWB-Offscreen] Frontend heartbeat timeout ({sinceHeartbeat:F1}s > {kFrontendHeartbeatTimeoutSec}s), frame#{s_OffscreenFrameCount}, stopping capture");
                        StopOffscreenCapture();
                        return;
                    }
                    // Warn if heartbeat is getting stale but not yet timed out.
                    if (sinceHeartbeat > kFrontendHeartbeatTimeoutSec * 0.5 && s_OffscreenFrameCount % 30 == 0)
                    {
                        CodelyLogger.LogWarning($"[NWB-Heartbeat] Stale: {sinceHeartbeat:F1}s since last heartbeat (timeout={kFrontendHeartbeatTimeoutSec}s)");
                    }
                }
            }
            catch (Exception ex)
        ...
        private static void ApplyPendingOffscreenResizeTarget()
        {
            ...
            if (converged || s_OffscreenResizeRetryFrames <= 0)
            {
                ...
                try
                {
                    ...
                    s_OffscreenTarget.SendEvent(new Event { type = EventType.Repaint });
                    s_OffscreenTarget.SendEvent(new Event { type = EventType.Layout });
                    s_OffscreenTarget.SendEvent(new Event { type = EventType.Repaint });
                }
                catch (Exception) { }
        ...
        {
            if (s_OffscreenTarget == null || s_OffscreenTargetType == null) return;
            if (s_OffscreenTargetType.Name != "GameView") return;
            ...
            try
            {
                ...
                if (s_CachedTargetTextureField == null) return;

                var tt = s_CachedTargetTextureField.GetValue(s_OffscreenTarget) as RenderTexture;
                if (tt == null || !tt.IsCreated()) return;

                float correctAspect = (float)tt.width / tt.height;

                // Reset all active cameras whose aspect was corrupted by
                // OnWindowSizeHasChanged() during SendEvent.
                foreach (Camera cam in Camera.allCameras)
                {
                    if (cam.targetTexture != null) continue;
        ...
        private static void CaptureAndPushCompositeFrame()
        {
            ...
            if (s_OffscreenFrameCount <= 3 || s_OffscreenFrameCount % 600 == 0)
            {
            ...
            foreach (var pair in viewSlots)
            {
                ...
                try
                {
                    ...
                    // TryCaptureViaGUIView may change RenderTexture.active while
                    // reading the slot. Always restore the composite target before
                    // drawing the slot texture into the final frame.
                    RenderTexture.active = s_CompositeRT;
        ...
        {
            if (s_MenuGetMenuItemsMethod == null) return false;
            try
            {
        ...
        /// <summary>
        /// Cancel the native delayed context menu queued by C++ MenuController.
        /// On Windows, DisplayCustomMenu queues a delayed popup (gDelayedContextMenu).
        /// When the next SendEvent (Layout/Repaint) fires, C++ GUIView::SendEvent
        /// calls ShowDelayedContextMenu → TrackPopupMenuEx, showing a native popup.
        /// By removing the menu item beforehand, FindItem returns null and
        /// ShowDelayedContextMenu cleans up without calling TrackPopupMenuEx.
        /// </summary>
        /// <summary>
        /// Dismiss an active Win32 TrackPopupMenuEx modal loop if one was already
        /// shown on the local desktop (outside the video stream). WM_CANCELMODE
        /// cancels modal menu loops without requiring a click on the native menu.
        /// </summary>
        private static void TryDismissWin32TrackPopupMenu()
        {
            ...
            try
            {
                foreach (IntPtr hwnd in GetAllUnityHwnds())
                {
                    PostMessageW(hwnd, WM_CANCELMODE, IntPtr.Zero, IntPtr.Zero);
                }
                LogVerbose("[NWB-ContextMenu] Posted WM_CANCELMODE to dismiss native TrackPopupMenuEx");
            }
            catch (Exception ex)
            {
                CodelyLogger.LogWarning($"[NWB-ContextMenu] WM_CANCELMODE dismiss failed: {ex.Message}");
            }
#endif
        }
        ...
        private static void CancelNativeDelayedPopup(bool forceRemove = false)
        {
            ...
            try
            {
                // Context menus forwarded to the HTML overlay must keep
                // MenuController entries until popup_select executes them.
                if (!forceRemove && s_FrontendPopupSent &&
                    (s_PopupSourceType == "generic_context" || s_PopupSourceType == "generic"))
                {
                    LogVerbose("[NWB-PopupCancel] Skipping MenuController removal — awaiting popup_select");
                    return;
                }
            ...
            Rect cwPos, Rect targetScreenPos, float ppp)
        {
            ...
            try
            {
                ...
                {
                    CodelyLogger.LogWarning("[NWB-FrontendPopup] Menu.GetMenuItems method not found");
                    return false;
                }
                ...
                // Build a lightweight signature to detect stale data.
                string firstPath = "";
                var smiType = menuItems.GetValue(0).GetType();
                var pp = smiType.GetProperty("path", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (pp != null)
                    firstPath = pp.GetValue(menuItems.GetValue(0)) as string ?? "";
                string sig = $"{menuItems.Length}|{firstPath}";
        ...
        // When true, keep MenuController items alive for ExecuteMenuItem and
        // suppress target Repaint until popup_close/select to avoid
        // ShowDelayedContextMenu → TrackPopupMenuEx on Windows.
        ...
        private static bool SendGenericMenuPopup(System.Array menuItems, System.Type menuType,
            ...
            bool hierarchicalContextMenu = false, bool deferMenuRemoval = false)
        {
            ...
            if (hierarchicalContextMenu)
            {
                List<PopupMenuTreeNode> roots = BuildHierarchicalPopupMenuTree(
                    ...
                    getCheckedMethod, getEnabledMethod);
            ...
            if (sent)
            {
                ...
                // For context menus (deferMenuRemoval), keep MenuController entries
                // alive until popup_select executes them. Repaint suppression blocks
                // ShowDelayedContextMenu → TrackPopupMenuEx without removing items.
                if (!deferMenuRemoval)
                    CancelNativeDelayedPopup();
                else
                    LogVerbose($"[NWB-FrontendPopup] Context menu forwarded id={s_CurrentFrontendPopupId} (MenuController retained until select/close)");
            ...
            float clickX, float clickY, EventModifiers mods, Vector2 mousePos, Vector2 mouseDelta)
        {
            ...
            try
            {
                ...
                LogVerbose($"[NWB-ContextClick] Synthesized ContextClick at ({mousePos.x:F0},{mousePos.y:F0}) for {s_OffscreenTargetType?.Name}");
            }
            ...
            if (!s_FrontendPopupSent)
                TryForwardMenuControllerPopup(clickX, clickY, forceResend: true);
            ...
            if (s_FrontendPopupSent)
            {
                // MenuController items are retained (deferMenuRemoval) for popup_select.
                TryDismissWin32TrackPopupMenu();
                s_PendingNativePopupCloseFrames = Math.Max(s_PendingNativePopupCloseFrames, 60);
            ...
            s_SuppressTargetRepaintWhilePopupOpen = false;
            // Leave MenuController intact so TryInterceptDelayedPopup /
            // TryInterceptGenericMenuPopup can still run on this mouseup.
            return false;
        }
        ...
        private static void HandleFrontendPopupSelect(string json)
        {
            try
            {
                ...
                {
                    LogVerbose($"[NWB-FrontendPopup] Executing delayed popup selection index={index}");
                    HandleDelayedPopupSelect(index);
                    ClearFrontendPopupState();
                    return;
                }
            ...
            object prevCallbackInstance, float clickX, float clickY, float ppp)
        {
            ...
            try
            {
                ...
                if (getMenuItemsMethod == null)
                {
                    CodelyLogger.LogWarning("[NWB-DelayedPopup] Menu.GetMenuItems not found");
                    return false;
                }
                ...
                // Position popup below the toolbar at the click X coordinate.
                float popX = clickX;
                float toolbarBottom = s_TabBarOffsetY + 20f;
                float popY = Mathf.Max(toolbarBottom, clickY + 4f);
        ...
        private static bool TryForwardMenuControllerPopup(float clickX, float clickY, bool forceResend)
        {
            ...
            try
            {
                ...
                MethodInfo getMenuItemsMethod = menuType.GetMethod("GetMenuItems",
                    ...
                    new System.Type[] { typeof(string), typeof(bool), typeof(bool) },
                    null);
                if (getMenuItemsMethod == null) return false;
                ...
                float popX = clickX;
                float popY = Mathf.Max(s_TabBarOffsetY + 4f, clickY + 4f);
                ...
                bool sent = SendGenericMenuPopup(
                    menuItems, menuType, popX, popY, 0, 0,
                    hierarchicalContextMenu: true, deferMenuRemoval: true);
                if (sent)
                {
                    s_PendingNativePopupCloseFrames = 60;
                ...
                else
                {
                    ...
                    // Prevent the upcoming Layout+Repaint from calling
                    // ShowDelayedContextMenu → TrackPopupMenuEx on Windows.
                    CancelNativeDelayedPopup();
                    CodelyLogger.LogWarning("[NWB-ContextMenu] Failed to send popup_show; cancelled native delayed menu");
                }
                return sent;
            }
            catch (Exception ex)
        ...
        private static void ScanAndCloseNativePopups()
        {
            ...
            try
            {
                ...
                foreach (var cw in allContainers)
                {
                    ...
                    int showMode = (int)showModeProp.GetValue(cw);
                    // Only handle PopupMenu (showMode==1) for frontend extraction.
                    // Utility(2), Tooltip(6), ModalUtility(7) are no longer
                    // pixel-composited; they are simply ignored here.
                    if (showMode != 1) continue;
        ...
        // Direct uGUI click injection state.
        ...
        private static void InjectInputEvent(string json)
        {
            ...
            try
            {
                ...
                if (type == "heartbeat")
                {
                    ...
                    if (wasFirst)
                    {
                        LogVerbose($"[NWB-Heartbeat] First heartbeat received at t={s_LastFrontendHeartbeatTime:F2}");
                        // Re-send current cursor lock state on new connection.
                        // The cursor_lock message may have been sent before the
                        // DataChannel was fully open, so the frontend missed it.
                ...
                if (s_CompositeActive)
                {
                    ...
                    object hitView = hasCompositeMouseCapture ? s_CompositeMouseCaptureView : null;
                    CompositeCaptureSlot hitSlot = hasCompositeMouseCapture ? s_CompositeMouseCaptureSlot : null;
                    if (hitView == null || isMouseDownEvent)
                    {
                        hitView = null;
                        hitSlot = null;
                    ...
                    if (hitSlot != null)
                    {
                        ...
                        s_OffscreenTargetGUIViewHwnd = hitSlot.GUIViewHwnd;
                    }
#endif
#if UNITY_EDITOR_OSX
                ...
                if (evt != null)
                {
                    ...
                    else if (!sentToPopup)
                    {
                            ...
                            if (isGamePlayMode)
                            {
                                ...
                                if (s_OffscreenTargetNSWindow != IntPtr.Zero)
                                {
                                    ...
                                    if (s_MacHeldKeys.Count > 0)
                                    {
                                        ...
                                        s_MacHeldKeys.RemoveWhere(k => k != keyForCommand);
                                    }
                                    objc_msgSend_void_bool(s_NSApp, s_ActivateIgnoringSel, true);
                                    objc_msgSend_void(s_OffscreenTargetNSWindow, s_MakeKeyWindowSel);
                        ...
                        else if (type == "mousedown")
                        {
                            ...
                            bool isTabHeaderClick = s_TabBarOffsetY > 0 && activeMousePos.y <= s_TabBarOffsetY;
                                ...
                                && rawY >= (s_TabBarOffsetY + 21f) * ppp)
                            {
                                ...
                                QueuePlayModeMouseEvent(EventType.MouseDown, rawX, rawY, button, mods);

                                // Path B: direct uGUI click injection
                                InjectStreamingUIPress(rawX, rawY, button);
                                ...
                                if (s_ActionEventCount <= 10)
                                {
                                    ...
                                    float gpY = rawY - (s_TabBarOffsetY + 21f) * pppDbg;
                                    LogVerbose($"[NWB-PlayMouse] mousedown QUEUED+UI: " +
                                        $"rawPx=({rawX},{rawY}) gamePx=({rawX:F0},{gpY:F0}) " +
                            ...
                            else
                            {
                                ...
                                bool isInTabHeader = s_TabBarOffsetY > 0
                                    && activeMousePos.y < s_TabBarOffsetY;
...
#endif
                                // Path A: queue IMGUI event
                                QueuePlayModeMouseEvent(EventType.MouseUp, rawX, rawY, button, mods);

                                // Path B: complete the uGUI press→click sequence
                                InjectStreamingUIRelease(rawX, rawY, button);
                        ...
                        else
                        {
                            ...
                            if (isGamePlayMode && (type == "mousemove" || type == "mousedrag"))
                            {
                                ...
                                QueuePlayModeMouseEvent(moveType, rawX, rawY, button, mods, activeMouseDelta);
                        ...
                        && !isContextMenuMouseUp)
                    {
...
#endif
                        {
                            ...
                            try
                            {
                                Event layoutEvt = new Event { type = EventType.Layout };
                                s_OffscreenTarget.SendEvent(layoutEvt);
                                Event repaintEvt = new Event { type = EventType.Repaint };
                    ...
                    if (s_InputEventCount <= 10 || (isAction && s_ActionEventCount <= 10))
                    {
                        ...
                        if (type == "mousedown" || type == "mouseup")
                            ...
                            extra = $" key='{evt.keyCode}' ch={(int)evt.character}('{(evt.character > 31 ? evt.character : '?')}') mod={evt.modifiers} kbCtrl={GUIUtility.keyboardControl} editTF={EditorGUIUtility.editingTextField}";
        ...
        // Track the last DockArea tab we switched to in composite mode.
        // When s_OffscreenTarget changes between slots sharing a DockArea,
        // the DockArea.selected must match so that Internal_SendEvent routes
        // events through the correct EditorWindow's UIElements tree.
        ...
        private static void EnsureLastActiveSceneView(SceneView sv)
        {
            ...
            if (s_LastActiveSceneViewField != null)
            {
                ...
                LogVerbose($"[NWB-SceneViewTool] Set lastActiveSceneView={sv.GetInstanceID()} via reflection");
            }
            else
            {
        ...
        private static bool ExtractJsonBool(string json, string key)
        {
            ...
            return pos < json.Length && json[pos] == 't'; // "true" starts with 't'
        }
        ...
        private static void InjectStreamingUIPress(float rawPixelX, float rawPixelY, int button)
        {
            ...
            try
            {
...
#if UNITY_EDITOR_WIN
                // Convert raw RT pixels (top-left origin, includes tab+toolbar)
                // to game screen coords (bottom-left origin, game area only).
                float ppp = EditorGUIUtility.pixelsPerPoint;
                float toolbarPx = (s_TabBarOffsetY + 21f) * ppp;
                float screenX = rawPixelX;
                float screenY = Screen.height - (rawPixelY - toolbarPx);
#else
                ...
                float screenX = rawPixelX;
                float screenY = Screen.height - rawPixelY;
#endif
                ...
                if (results.Count > 0)
                {
                    ...
                    else
                    {
                        LogVerbose($"[NWB-StreamUI] Press: {results.Count} hits at " +
                            $"screenPos=({screenX:F0},{screenY:F0}) but no IPointerClickHandler found");
                    }
                }
                else
                {
        ...
        /// send IPointerUpHandler + IPointerClickHandler to the previously
        /// pressed target.
        /// </summary>
            ...
            int btn, EventModifiers mod, Vector2 delta = default)
        {
            ...
            // GameView toolbar height = EditorGUI.kWindowToolbarHeight = 21f.
            const float kGVToolbar = 21f;
            float offsetPixels = (s_TabBarOffsetY + kGVToolbar) * pppLocal;
