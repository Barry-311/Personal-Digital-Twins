...
namespace UnityTcp.Editor.Tools
{
    ...
    public static class ManageEditor
    {
            ...
            switch (action)
            {
                ...
                case "step":
                    try
                    {
                        if (!EditorApplication.isPlaying)
                        {
                            // Auto-enter play mode and pause before stepping.
                            EditorApplication.isPlaying = true;
                        ...
                        return Response.Success("Stepped one frame.", new
                        {
                            playMode = "paused"
                        });
                    }
                ...
                default:
                    return Response.Error(
                        $"Unknown action: '{action}'. Supported actions include: get_current_state, request_compile, wait_for_compile, wait_for_idle, play, pause, stop, step, get_state, get_project_root, get_windows, get_active_tool, get_selection, set_active_tool, add_tag, remove_tag, get_tags, add_layer, remove_layer, get_layers, focus_window."
                    );
            }
        }
        ...
        private static object RequestCompile()
        {
            ...
            {
                CodelyLogger.LogError($"[ManageEditor] Failed to request compilation: {e}");
