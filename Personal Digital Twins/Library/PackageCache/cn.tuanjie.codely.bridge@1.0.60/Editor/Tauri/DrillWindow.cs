...
namespace Cn.Tuanjie.Codely.Editor
{
    ...
    public class DrillWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (_webView2Host != null && _webView2Host.IsReady)
            {
                ...
                int insetPx = Mathf.Max(0, Mathf.RoundToInt(
                    (origin.y - position.y) * EditorGUIUtility.pixelsPerPoint));
        ...
        private void AcquireIpcClient()
        {
            if (!_ipcAcquired)
            {
                if (_ipcOwnerId == null) _ipcOwnerId = $"DrillWindow:{GetInstanceID()}";
        ...
        private void OnDrillCompleted()
        {
            ...
            EditorApplication.delayCall += () =>
            {
                // Force CodelyWindow to use lock-based attach instead of SessionState restore.
                // This keeps the detach -> embed handshake path consistent after walkthrough.
                ...
                // Close first so DrillWindow's OnDestroy runs synchronously (IPC release,
                // WebView teardown) before the layout swap tears down every other window.
                Close();
