...
namespace Cn.Tuanjie.Codely.Editor
{
    ...
    public class CodelyWindow : EditorWindow
    {
        ...
        private static void CleanupForPackageRemovalOrUpgrade()
        {
            ...
            {
                KillTauriProcessFromSessionState();
            }
        ...
        private static void OnEditorQuitting()
        {
            ...
            foreach (var window in Resources.FindObjectsOfTypeAll<DrillWindow>())
                window?.ClearWebView2OnQuit();
#endif
            // Backward-compat: only the single-instance Tauri build is shared
            // across every Editor; a legacy build (one process per workspace)
            // still owns its own process and must be killed on quit to avoid
            // orphaning it.
            if (IsTauriSingleInstance())
            {
                // IMPORTANT: Do NOT kill the Tauri process here. In single-instance
                // mode the Tauri app serves every workspace from one process;
                // killing it on this Editor's quit would wipe out every other
                // Cowork window (the bug the user reported).
                // We only tear down this Editor's IPC client and clear its own
                // SessionState so a future launch of *this* Editor starts clean;
                // the surviving Tauri process keeps running for the other workspaces.
                StopIpcClientStatic();
                SessionState.EraseInt(SESSION_STATE_TAURI_PID_KEY);
                SessionState.EraseString(SESSION_STATE_TAURI_WINDOW_HANDLE_KEY);
                SessionState.EraseInt(SESSION_STATE_TAURI_PORT_KEY);
            ...
            // Legacy Tauri build: this Editor owns the process, so kill it now
            // (matches pre-single-instance behaviour). Stop IPC explicitly
            // first — `KillTauriProcessFromSessionState` also stops it in its
            // `finally`, but making it explicit here mirrors the
            // single-instance branch above and guarantees ordering.
            StopIpcClientStatic();
            KillTauriProcessFromSessionState();
            SessionState.EraseString(SESSION_STATE_TAURI_SINGLE_INSTANCE_KEY);
        }
        ...
        /// <summary>
        /// Full cleanup using only SessionState (no instance). Matches CleanupTauriProcess: detach/hide window, kill process, clear state, stop IPC.
        /// Called when package is removed or editor quits, when EditorWindow instance may be unavailable.
        /// </summary>
        private static void KillTauriProcessFromSessionState()
        {
            try
            {
                // Kill Tauri process by PID
                int pid = SessionState.GetInt(SESSION_STATE_TAURI_PID_KEY, -1);
                if (pid > 0)
                {
                    try
                        {
                    ...
                    {
                        CodelyLogger.LogWarning($"Codely: Could not kill Tauri process from SessionState: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                CodelyLogger.LogWarning($"Codely: KillTauriProcessFromSessionState error: {ex.Message}");
            }
            finally
            {
                SessionState.EraseInt(SESSION_STATE_TAURI_PID_KEY);
        ...
        // Cached capability advertised by `/api/tauri/status`:
        //   "1" → Tauri process is single-instance and shared across every
        //         Unity Editor in this user session; we must NOT kill it when
        //         one Editor quits or its CodelyWindow closes.
        //   "0" or missing → legacy Tauri build (one process per workspace);
        //         kill-on-close is safe and expected.
        // Stored in SessionState so it survives domain reloads and is readable
        // from static handlers (OnEditorQuitting, KillTauriProcessFromSessionState).
...
// #endif
        ...
        private void OnEnable()
        {
            ...
            // Theme: record current observed theme and mark as dirty so it will be sent once IPC is available
            ...
            if (!tauriRestored)
            {
                // 杀掉残留 Tauri 进程，释放 IPC 管道，使新进程能正常创建
                KillStaleTauriProcesses();
            }
        ...
        private async void CleanupResourcesAsync()
        {
            ...
            // reused by the next session). For a normal window close we still
            // skip killing the Tauri process *only when* it is a
            // single-instance build — it is shared across every Unity Editor /
            // Cowork window in this user session, so killing it here would
            // tear down every other workspace's UI (the bug the user
            // reported). For legacy (non-single-instance) Tauri builds, each
            // Editor owns its own process and we fall back to the old kill
            // behaviour so we don't orphan the process.
            bool preserveTauri = _preserveTauriProcessOnDestroy;
            _preserveTauriProcessOnDestroy = false;
            bool singleInstance = IsTauriSingleInstance();
            Process processToKill = (!preserveTauri && !singleInstance) ? _tauriProcess : null;
            _tauriProcess = null;
            _tauriServerPort = -1;
            SessionState.EraseInt(SESSION_STATE_TAURI_PID_KEY);
            SessionState.EraseInt(SESSION_STATE_TAURI_PORT_KEY);
            ...
            // For legacy (non-single-instance) builds, kill the Tauri process
            // synchronously on the main thread *before* any await so that a
            // new window opening immediately afterwards cannot find a stale
            // lock file and attach to the still-running old process (which
            // would then be killed, taking the new window's server with it).
            if (processToKill != null)
            {
                try
                {
                    ...
                    CodelyLogger.Log("Codely: Legacy Tauri build detected — killed locally-owned Tauri process on window close");
                }
                catch (Exception ex)
                {
                    CodelyLogger.LogWarning($"Codely: Error killing Tauri process: {ex.Message}");
                }
            }

            // Background-thread cleanup: stop IPC client. For single-instance
            // builds the shared Tauri process is owned app-wide and survives
            // this window's teardown; when its last workspace closes, Tauri
            // exits on its own via its window-close-requested handlers.
        ...
        private async System.Threading.Tasks.Task LaunchTauriApplicationInternal(string unityAccessToken, string unityUserId)
        {
            try
            {
                ...
                // Second stale-process cleanup; stragglers can appear during earlier launch steps.
                KillStaleTauriProcesses();

                if (TryAttachToExistingProjectTauriFromLock())
                {
                    CodelyLogger.Log("Codely: Existing Tauri process for this workspace detected after cleanup, skip launch.");
                    return;
                }

                if (await TryAttachViaRunningSingleInstanceAppAsync())
                {
                    CodelyLogger.Log("Codely: Attached to existing single-instance APP after cleanup; skip launch.");
                    return;
                }
        ...
        /// <summary>
        /// Reads the cached single-instance capability flag. Defaults to
        /// <c>false</c> when the probe has not yet succeeded — callers treat
        /// that as "legacy behaviour" (kill Tauri on close).
        /// </summary>
        private static bool IsTauriSingleInstance()
        {
            return SessionState.GetString(SESSION_STATE_TAURI_SINGLE_INSTANCE_KEY, "0") == "1";
        }
            ...
            if (savedPid > 0)
            {
                try
                {
                    ...
                    if (!process.HasExited)
                    {
                        ...
                        // Refresh single-instance capability so subsequent
                        // cleanup paths (KillStaleTauriProcesses, etc.) know
                        // this is a shared process that must not be killed.
                        if (_tauriServerPort > 0)
                        {
                            RefreshTauriSingleInstanceCapability(_tauriServerPort);
                        }
        ...
        /// <summary>
        /// Cleans stale Tauri processes only for the current workspace context.
        /// </summary>
        private void KillStaleTauriProcesses()
        {
            // In single-instance mode the APP process is shared by every
            // workspace. Killing it here would tear down the UI for all other
            // workspaces — leave it alone.
            if (IsTauriSingleInstance())
            {
                return;
            }
            ...
            try
            {
                ...
                int sessionPid = SessionState.GetInt(SESSION_STATE_TAURI_PID_KEY, -1);
                if (sessionPid > 0)
                {
                    candidatePids.Add(sessionPid);
                }
                ...
                if (File.Exists(lockFilePath))
                {
                    try
                    {
                        string lockContent = File.ReadAllText(lockFilePath);
                        int lockPid = TryExtractIntFieldFromJson(lockContent, "pid");
                        if (lockPid > 0)
                        {
                            candidatePids.Add(lockPid);
                        }
                    }
                    catch (Exception ex)
                    {
                        CodelyLogger.LogWarning($"Codely: Failed to read lock file before stale cleanup ({lockFilePath}): {ex.Message}");
                    }
                }

                foreach (int pid in candidatePids)
                {
                    if (_tauriProcess != null && pid == _tauriProcess.Id)
                        continue;
                    ...
                    try
                    {
                        ...
                        CodelyLogger.Log($"Codely: Killed stale Tauri process for current workspace (PID: {pid})");
                    }
                    catch (ArgumentException)
                    {
                        // Process already exited
                    ...
                    {
                        CodelyLogger.LogWarning($"Codely: Failed to kill stale Tauri process (PID: {pid}): {ex.Message}");
                    }
                    finally
                    {
                        process?.Dispose();
            ...
            {
                CodelyLogger.LogWarning($"Codely: KillStaleTauriProcesses failed: {ex.Message}");
            }
        }
        ...
        private void CleanupTauriProcess()
        {
            ...
            if (_tauriProcess != null && !_tauriProcess.HasExited)
            {
                ...
                {
                    _tauriProcess.Kill();
                    _tauriProcess.WaitForExit(1000);
                }
                catch (Exception ex)
                {
                    CodelyLogger.LogWarning($"Codely: Error killing Tauri process: {ex.Message}");
        ...
        private void AcquireIpcClient()
        {
            if (!_ipcAcquired)
            {
                if (_ipcOwnerId == null) _ipcOwnerId = $"CodelyWindow:{GetInstanceID()}";
        ...
        /// Does not kill the Tauri process — only clears Unity-side state (same as normal close minus Kill).
        /// </summary>
