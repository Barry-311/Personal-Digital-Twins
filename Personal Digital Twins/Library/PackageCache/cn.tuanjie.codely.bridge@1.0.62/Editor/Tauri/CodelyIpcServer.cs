...
namespace Cn.Tuanjie.Codely.Editor
{
    ...
    public static class IpcMessageType
    {
        ...
        public const string WebRtcStartSession = "webrtcStartSession";
        public const string WebRtcStopSession = "webrtcStopSession";
        public const string WebRtcInput = "webrtcInput";
        public const string WebRtcSessionReady = "webrtcSessionReady";
        public const string WebRtcSessionStopped = "webrtcSessionStopped";
        public const string WebRtcSessionError = "webrtcSessionError";
    }
    ...
    public class CodelyIpcClient : IDisposable
    {
                ...
                EditorApplication.delayCall += () =>
                {
                    switch (message.messageType)
                    {
                        ...
                        case IpcMessageType.UpdateRestart:
                            ...
                            break;
                        case IpcMessageType.WebRtcStartSession:
                            HandleWebRtcStartSession(message.data);
                            break;
                        case IpcMessageType.WebRtcStopSession:
                            HandleWebRtcStopSession();
                            break;
                        case IpcMessageType.WebRtcInput:
                            HandleWebRtcInput(message.data);
                            break;
        ...
        private void HandleWebRtcStartSession(string data)
        {
            try
            {
                // Keep IPC decoupled from UnityTcp.Editor asmdef via reflection.
                string responseJson = InvokeStaticStringMethod(
                    "UnityTcp.Editor.Tools.ManageWebRtcStream, UnityTcp.Editor",
                    "StartSessionFromIpc",
                    data ?? "{}"
                );
                if (string.IsNullOrEmpty(responseJson))
                {
                    responseJson = "{\"success\":false,\"error\":\"Empty response from ManageWebRtcStream.StartSessionFromIpc\"}";
                }
                // IndexOf instead of Contains(string, StringComparison) for .NET Standard 2.0 compat
                bool hasFailure = responseJson.IndexOf("\"success\":false", StringComparison.OrdinalIgnoreCase) >= 0;
                SendMessage(
                    hasFailure ? IpcMessageType.WebRtcSessionError : IpcMessageType.WebRtcSessionReady,
                    responseJson
                );
            }
            catch (Exception ex)
            {
                string errorPayload = $"{{\"success\":false,\"error\":\"{EscapeForJson(ex.Message)}\"}}";
                SendMessage(IpcMessageType.WebRtcSessionError, errorPayload);
                CodelyLogger.LogError($"Codely IPC: Error handling WebRtcStartSession: {ex.Message}");
            }
        }

        private void HandleWebRtcStopSession()
        {
            try
            {
                string responseJson = InvokeStaticStringMethod(
                ...
                SendMessage(IpcMessageType.WebRtcSessionStopped, responseJson ?? "{\"success\":true}");
            }
            catch (Exception ex)
            {
                string errorPayload = $"{{\"success\":false,\"error\":\"{EscapeForJson(ex.Message)}\"}}";
                SendMessage(IpcMessageType.WebRtcSessionError, errorPayload);
                CodelyLogger.LogError($"Codely IPC: Error handling WebRtcStopSession: {ex.Message}");
            }
        }

        private void HandleWebRtcInput(string data)
        {
            try
            {
                bool ok = InvokeStaticBoolMethod(
                    "UnityTcp.Editor.Tools.ManageWebRtcStream, UnityTcp.Editor",
                    "InjectInputFromIpc",
                    data ?? "{}"
                );
                if (!ok)
                {
                    CodelyLogger.LogWarning("Codely IPC: WebRTC input injection returned false.");
                }
            }
            catch (Exception ex)
            {
                CodelyLogger.LogError($"Codely IPC: Error handling WebRtcInput: {ex.Message}");
            }
        }

        private static string InvokeStaticStringMethod(string typeName, string methodName, params object[] args)
        {
        ...
        private static bool InvokeStaticBoolMethod(string typeName, string methodName, params object[] args)
        {
            Type t = Type.GetType(typeName, throwOnError: false);
            if (t == null)
            {
        ...
        // Escape all JSON-unsafe characters including control chars (U+0000–U+001F).
