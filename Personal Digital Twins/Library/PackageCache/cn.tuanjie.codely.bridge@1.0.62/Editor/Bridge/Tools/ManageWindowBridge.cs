    ...
    public static class ManageWindowBridge
    {
        ...
        private static object SendInput(JObject @params)
        {
            ...
            if (eventType == "focus")
            {
                ...
                return Response.Success(
                    "Focus input applied.",
                    new { windowId = target.GetInstanceID(), eventType }
                );
            }
        ...
        {
            int? instanceId = @params?["instanceID"]?.ToObject<int?>() ?? @params?["windowId"]?.ToObject<int?>();
            ...
            {
                EditorWindow byId = allWindows.FirstOrDefault(w => w != null && w.GetInstanceID() == instanceId.Value);
        ...
        private static void PushQueuedWindowListToNativeTick()
        {
            ...
            if (PushWindowListToNative())
            {
                ...
                CodelyLogger.Log($"[NWB] Editor window list changed via WinEventHook; notifiedFrontend={notified}");
            }
        }
        ...
        private static object StopOffscreenStream()
        {
            ...
            return Response.Success("Offscreen capture stopped.", new { mode = "stopped" });
        }
