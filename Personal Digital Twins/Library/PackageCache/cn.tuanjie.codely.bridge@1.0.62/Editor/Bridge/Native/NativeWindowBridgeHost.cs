    ...
    internal static partial class NativeWindowBridgeHost
    {
...
#if UNITY_EDITOR_WIN
                        // PrintWindow path for Unity 2019 Gamma — captures
                        // complete window content (tab bar + toolbar + content)
                        // via DWM composition, no GrabPixels dependency.
                        if (s_UseRenderTextureFallback && slot.ContainerHwnd != IntPtr.Zero)
                            captured = TryCaptureViaPrintWindow(slot.ContainerHwnd, s_CaptureRT);
#endif
                        if (!captured)
            ...
            object prevCallbackInstance, float clickX, float clickY, float ppp)
        {
            ...
            try
            {
                ...
                if (getMenuItemsMethod == null)
                {
                    ...
                    float fallbackToolbarBottom = s_TabBarOffsetY + 20f;
                    float fallbackPopY = Mathf.Max(fallbackToolbarBottom, clickY + 4f);
                    bool fallbackSent = SendDelayedPopupViaSubmenus(
                        clickX * ppp, fallbackPopY * ppp, ppp);
                    if (fallbackSent)
                    {
        ...
        private static bool TryForwardMenuControllerPopup(float clickX, float clickY, bool forceResend)
        {
            ...
            try
            {
                ...
                float fbPopX = clickX;
                float fbPopY = Mathf.Max(s_TabBarOffsetY + 4f, clickY + 4f);
                bool fbSent = SendDelayedPopupViaSubmenus(fbPopX, fbPopY, 1f);
                if (fbSent)
                {
        ...
        private static bool TryCaptureViaPrintWindow(IntPtr containerHwnd, RenderTexture captureRT)
        {
            ...
            int copyW = Mathf.Min(clientW, dstW);
            int copyH = Mathf.Min(clientH, dstH);
            if (copyW <= 0 || copyH <= 0) return false;

            int dataSize = dstW * dstH * 4;
            ...
            {
                Marshal.Copy(
                    s_PwBits + (borderT + y) * srcStride + borderL * 4,
                    s_PwBuffer, y * dstStride, rowBytes);
            }

            // Fix alpha: PrintWindow may leave alpha at 0 for opaque pixels
            // (GDI convention). Force alpha to 255 so the texture is opaque.
            for (int i = 3; i < copyH * dstStride; i += 4)
                s_PwBuffer[i] = 255;

            if (s_PwTex == null || s_PwTex.width != dstW || s_PwTex.height != dstH)
            {
                if (s_PwTex != null) UnityEngine.Object.DestroyImmediate(s_PwTex);
                s_PwTex = new Texture2D(dstW, dstH, TextureFormat.BGRA32, false);
            }
